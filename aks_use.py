...
def main():
    ...
    # set the buy/sell price, win/loss counts to calculate the win rate
    # buy_price, sell_price = 0, 0
    win_count, loss_count = 0, 0
    avg_win, avg_loss = 0, 0
    profit_factor = 0  # Profit factor = total profit / total loss
    win_rate = 0  # Win rate = win_count / (win_count + loss_count)
    ...
    buy_trade_records = []  # List to record trade transactions
    sell_trade_records = []  # List to record trade transactions
    ...
    total_executed = len(buy_trade_records) + len(sell_trade_records)
    ...
    for i in range(len(buy_trade_records)):
        ...
        if sell_price is not None:
            if sell_price > buy_price:
                win_count += 1
                avg_win += (sell_price - buy_price) * buy_trade_records[i]['shares']
            else:
                loss_count += 1
                avg_loss += (buy_price - sell_price) * buy_trade_records[i]['shares']
